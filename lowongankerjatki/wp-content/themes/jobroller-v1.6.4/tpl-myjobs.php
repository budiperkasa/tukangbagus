	...
	<div class="section myjobs">
		...
		<?php
			...
			if (!empty($pack)):
				...
				$resume_temp_access .= in_array('resume_view', $pack['access']) ? ($resume_temp_access?__(' and ', 'appthemes'):'') . __('View','appthemes') : '';			
			endif;
		?>
		...
		<div id="live" class="myjobs_section">
			...
			<div id="subscriptions" class="myjobs_section">
				...
				<?php if ($resume_temp_access) { ?>
					...
					<p><?php echo sprintf( __('Your purchased Job Pack(s) give you temporary access to <strong>%s</strong> Resumes. ***','appthemes'), $resume_temp_access ); ?></p>
					<p class="temp_subsc_info"><em><?php echo __('*** Access is granted for the duration of the Pack (limited duration Packs) or while the Pack is active (unlimited duration Packs).','appthemes'); ?></em></p>
