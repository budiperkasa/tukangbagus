...
class jr_order {
	...
	function set_pack_data( $pack_id ) {
		...
		$result = $wpdb->get_var( $wpdb->prepare( "SELECT pack_id FROM $wpdb->jr_customer_packs WHERE id = %d", $pack_id ) );
		if ( $result ) {
			$this->pack_type = 'user_pack';
			$this->pack_type_desc = __('User Pack ::', 'appthemes');
		} else {
			$this->pack_type = 'new_pack';
			$this->pack_type_desc = __('New Pack ::', 'appthemes');
		}
	}
		...
		if ($this->pack_id > 0) :
			...
			if ( 'user_pack' == $this->pack_type ) {
				...
				if ( ! $pack->get_valid_pack( $this->user_id, $pack_status = 'any' ) ) {
					wp_die( __('Error: Invalid User Pack.', 'appthemes') );
					$jr_log->write_log( __('Error: Invalid User Pack.', 'appthemes') );
				}
			} else {
