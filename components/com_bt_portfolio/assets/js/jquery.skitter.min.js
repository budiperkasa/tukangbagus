...
(function ($) {
	...
	$.skitter = function(obj, options, number) {
		...
		function responsive(){
			var paddingWidth = self.box_skitter.outerWidth()-self.box_skitter.width();
			var width = self.box_skitter.parent().width()-paddingWidth;
			var height = Math.round(width/self.ratio);
			self.box_skitter.css({height:height,width:width});
	...
	$sk.fn.extend({
		setup : function () {
				...
				if (self.settings.thumbs) {
					...
					if(!src_thumb){
						...
						if (self.settings.width_skitter > self.settings.height_skitter) {
							...
							dimension_thumb = 'width="'+self.settings.with_thumb+'"'
						}
					}
			...
			if (self.settings.thumbs) {
				...
				var width_info_slide = (u+1)*parseInt(self.settings.with_thumb);
				...
				self.box_skitter.find('.container_thumbs').width(self.settings.width_skitter).height(self.settings.height_thumb + 10).append(copy_info_slide);
				...
				this.box_skitter.mousemove(function(e){  
					if (width_info_slide >self.settings.width_skitter) {  
						...
						if (new_x > 0) new_x = 0;
						if (new_x < -(w_info_slide_thumb - self.settings.width_skitter - 5)) new_x = -(w_info_slide_thumb - self.settings.width_skitter - 5);
			...
			if (this.settings.images_links.length > 1) {
				...
				if (self.settings.preview && self.settings.dots) {
					...
					var width_preview_ul = parseInt(this.settings.images_links.length * self.settings.with_thumb);
					preview.find('ul').width(width_preview_ul);
					$(class_info).append(preview);
					self.box_skitter.find(class_info).find('.image_number').mouseenter(function () {
						var _left_info = parseFloat(self.box_skitter.find(class_info).offset().left);
						var _left_image = parseFloat($(this).offset().left);
						var _left_preview = (_left_image - _left_info) - self.settings.with_thumb/2 + $('.info_slide_dots span').width()/2;
						var rel = parseInt($(this).attr('rel'));
						var image_current_preview = self.box_skitter.find('#preview_slide_current img').attr('src');
						var _left_ul =  - (rel * self.settings.with_thumb);
