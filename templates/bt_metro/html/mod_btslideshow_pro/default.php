...
$titleCSS = '#mod_btslideshow_pro_' . $moduleID . ' .label_skitter h4,
            ...
            {' .
        ($params->get('title_font') ? '   font-family: Open Sans;' : '') .
    ...
    <ul>
        ...
        foreach ($items as $item) {
            if (!empty($item->link)) {
                ...
                $img = '<a href="#cubeRandom"><img class="cubeRandom" src="' . $item->mainImage . '" 
                            rel="' . $item->thumbImage . '" 
                            alt= "' . $item->title . '"/></a>';
            }
            $desc = '';
...
<script type="text/javascript">
	...
	if ($params->get('navigation_type') == 'dots' || $params->get('navigation_type') == 'numbers') {
		...
		echo "BTJ('#mod_btslideshow_pro_{$module->id}').css({'margin-bottom':'0px'});\n\t\t";
	}
	?>
