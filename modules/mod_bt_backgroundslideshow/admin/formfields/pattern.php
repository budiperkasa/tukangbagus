...
class JFormFieldPattern extends JFormField {
    ...
    public function getInput(){
        ...
        $html = '<div class="bg-pattern">';
		if($this->value == ''){
			$html.= '<div class="active bg-pattern-item" title="" onclick="changePattern(this)">None</div>';
		}else{
			$html.= '<div class="bg-pattern-item" title="" onclick="changePattern(this)">None</div>';
		}
		foreach ($images as $image) {
			if($this->value == $image){
				...
				$html .= '<div onclick="changePattern(this)" class="bg-pattern-item" title="'.$image.'" style="background:#fff url('.JURI::root().$relative_path.$image.') left top repeat"></div>';
			}
		}
        $html .= '</div>';
