					...
					<div class="bt-row <?php if($i==0) {echo 'bt-row-first'; if($activateFirst) echo ' actived';} if($i==count($list)-1) echo ' bt-row-last';  ?>"  >
						...
						<?php if( $showTitle ): ?>
							<a class="bt-title" target="<?php echo $openTarget; ?>" title="<?php echo $row->title; ?>" href="<?php echo $row->link;?>"> 
								...
								<?php if($showArrow){?>
								<img src="<?php echo JURI::root() . 'modules/mod_bt_contentshowcase/tmpl/layouts/accordion/images/button.png' ?>" alt=""/>
								<?php }?>
							</a>
						<?php endif; ?>
...
<script type="text/javascript">
    $B(document).ready(function(){
        ...
        jQuery(moduleID + ' .bt-row.actived').find('.bt-inner-wrapper').show(0, function(){
                $B(this).prev().find('img').attr('src', '<?php echo JURI::root() . 'modules/mod_bt_contentshowcase/tmpl/layouts/accordion/images/button-active.png' ?>');
            }
        ); 
		<?php } ?>
    ...
    function btContentShowcaseAjustHeight(carousel, originalHeight){
		...
		*/
        ...
        if(carousel.options.vertical){
            ...
            btInnerWrapper.prev().find('img').attr('src', '<?php echo JURI::root() . 'modules/mod_bt_contentshowcase/tmpl/layouts/accordion/images/button.png' ?>');
        ...
        if(!$B(this).parent().parent().hasClass('actived')){
            ...
            $B(this).parent().parent().parent().find('.actived')
                .removeClass('actived').find('.bt-inner-wrapper').slideUp(300, function(){
                    $B(this).prev().find('img').attr('src', '<?php echo JURI::root() . 'modules/mod_bt_contentshowcase/tmpl/layouts/accordion/images/button.png' ?>');
